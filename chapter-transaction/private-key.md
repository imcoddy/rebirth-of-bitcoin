...
比特币的私钥（k）是一个**随机数**，可以是 11 和 (n−1)(n−1) 之间的任何数字，其中 nn 是常数 1.158×10771.158×1077，略小于 22562256。

生成一个私钥，本质上就是选择一个数。只要选取的方法不可预测或不可重复，它就是密码学安全的。

我们可以用不同的表示法，来表示同一个私钥。

格式值十六进制原始数据（Raw）f97c89aaacf0cd2e47ddbacc97dae1f88bec49106ac37716c451dcdd008a4b62WIF不压缩格式（WIF uncompressed）5KiANv9EHEU4o9oLzZ6A7z4xJJ3uvfK2RLEubBtTz1fSwAbpJ2UWIF压缩格式（WIF compressed）L5agPjZKceSTkhqZF2dmFptT5LFrbr6ZGPvP7u4A6dvhTrr71WZ9
...
[Base58](https://zh.wikipedia.org/wiki/Base58) 是 Base64 编码格式的子集，舍弃了一些容易错读和在特定字体中容易混淆的字符，不使用 Base64 中的`0`（数字0）、`O`（大写字母o）、`l`（小写字母L）、`I`（大写字母i）以及`+`和`/`，是比特币系统中使用的一种独特的编码方式。
...
[![](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0406.png)](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0406.png)

对于不同类型的数据，在做 Base58Check 编码时会添加不同的版本前缀，以产生易于辨识的不同结果。

类型版本前缀的值（十六进制）Base58Check之后的前缀P2PKH地址001P2SH地址053测试网络（testnet）地址6Fm 或 nWIF格式的私钥805，K 或 L根据BIP-38标准 加密的私钥01426P根据BIP-32标准定义的 扩展公钥0488B21Expub
...
对 Raw 格式的私钥`f97c89aaacf0cd2e47ddbacc97dae1f88bec49106ac37716c451dcdd008a4b62`，计算其 **WIF不压缩** 格式：
...
稍加改变，在私钥后添加一个压缩标志位，计算结果就是 **WIF压缩** 格式。
...
* [不压缩WIF与Raw格式私钥的转换工具](https://gobittest.appspot.com/PrivateKey)
* [十六进制Hash工具](https://www.fileformat.info/tool/hash.htm)
* [十六进制Base58编码工具](https://incoherency.co.uk/base58/)
...
y2=x3−x+1y2=x3−x+1 定义了一条椭圆曲线，函数图像如下。

[![Imgur](https://aaron67-public.oss-cn-beijing.aliyuncs.com/0MGgi5G.png)](https://aaron67-public.oss-cn-beijing.aliyuncs.com/0MGgi5G.png)

比特币使用的椭圆曲线，为

y2=x3+7\\mathchoice(modp)y2=x3+7\\mathchoice(modp)

这条椭圆曲线由 [Secp256k1](https://en.bitcoin.it/wiki/Secp256k1) 标准定义，其中

* mod 是取模运算（取余数），1=5\\mathchoice(mod2)1=5\\mathchoice(mod2)
* p 是定值 2256−232−29−28−27−26−24−12256−232−29−28−27−26−24−1，这是一个非常大的素数

因为方程两端取模运算的存在，所以这条曲线的函数图像并不是连续的，而是二维空间上一系列散开的点。

如果 p=17p=17，它的函数图像如下。

[![Imgur](https://aaron67-public.oss-cn-beijing.aliyuncs.com/79rRrpL.png)](https://aaron67-public.oss-cn-beijing.aliyuncs.com/79rRrpL.png)

你可以把 Secp256k1 的函数图像，想象成一个**极大的网格上一系列更为复杂的散点**。

椭圆曲线上的两点 p1 和 p2，定义 p=p1+p2p=p1+p2 为椭圆曲线上**点的加法运算**。

1. 过点 p1 和 p2 做直线与椭圆曲线相交点 q
2. 以 X 轴对称翻转点 q，得到点 p

[![Imgur](https://aaron67-public.oss-cn-beijing.aliyuncs.com/5FstmO4.png)](https://aaron67-public.oss-cn-beijing.aliyuncs.com/5FstmO4.png)
...
k×p=p+p+…+pkk×p=p+p+…+p⏟k

下图展示了从椭圆曲线上的点 G，计算 2G、4G、8G 的操作。

[![](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0404.png)](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0404.png)
...
[![](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0407.png)](https://github.com/bitcoinbook/bitcoinbook/raw/develop/images/mbc2_0407.png)
